...
public class StackHelper {
    ...
    /**
     * Compares two ItemStack instances if they reference the same object or are both null. If false, the method
     * areEqual() is called to check if the two instances reference the same item.
     */
    ...
    public static boolean matches(IIngredient ingredient, IItemStack itemStack) {
        ...
        if (ingredient.getItems() != null && TweakerPlugin.isLoaded("Botania")
                && Botania.isSubtile(toStack(itemStack))) {
    ...
    /**
     * This function compares an ingredient with a fluid. MCLiquidStack.matches() function is currently broken in
     * MineTweaker3, thus this function is only a workaround.
     */
