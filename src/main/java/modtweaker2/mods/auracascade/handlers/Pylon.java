...
public class Pylon {
    ...
    @ZenMethod
    public static void addRecipe(IItemStack ouput, IAuraStack aura1, IItemStack input1, IAuraStack aura2,
            IItemStack input2, IAuraStack aura3, IItemStack input3, IAuraStack aura4, IItemStack input4) {
        List<PylonRecipe> recipes = new LinkedList<PylonRecipe>();

        recipes.add(
                new PylonRecipe(
                        toStack(ouput),
                        new PylonRecipeComponent(AuraCascadeHelper.toAura(aura1), toStack(input1)),
                        new PylonRecipeComponent(AuraCascadeHelper.toAura(aura2), toStack(input2)),
                        new PylonRecipeComponent(AuraCascadeHelper.toAura(aura3), toStack(input3)),
                        new PylonRecipeComponent(AuraCascadeHelper.toAura(aura1), toStack(input4))));
    ...
    public static void removeRecipe(IIngredient output) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", Pylon.name, output.toString()));
        }
    }
