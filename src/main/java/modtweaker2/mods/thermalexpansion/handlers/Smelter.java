...
public class Smelter {
    ...
    @ZenMethod
    public static void addRecipe(int energy, IItemStack input, IItemStack input2, IItemStack output,
            @Optional IItemStack output2, @Optional int chance) {
        ...
        if (SmelterManager.recipeExists(toStack(input), toStack(input2))) {
            LogHelper.logWarning(
                    String.format(
                            ...
                            LogHelper.getStackDescription(toStack(input2))));
            return;
        }

        RecipeSmelter recipe = ReflectionHelper.getInstance(
                ThermalHelper.smelterRecipe,
                toStack(input),
                toStack(input2),
                toStack(output),
                toStack(output2),
    ...
    public static void removeRecipe(IIngredient input2, IIngredient input1) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipes found for %s and %s.", name, input1.toString(), input2.toString()));
        }
    }
