...
public class Furnace {
    ...
    public static void addRecipe(int energy, IItemStack input, IItemStack output) {
        ...
        RecipeFurnace recipe = ReflectionHelper
                .getInstance(ThermalHelper.furanceRecipe, toStack(input), toStack(output), energy);
    ...
    private static class Add extends BaseListAddition<RecipeFurnace> {
        ...
        public void apply() {
            for (RecipeFurnace recipe : recipes) {
                boolean applied = FurnaceManager
                        .addRecipe(recipe.getEnergy(), recipe.getInput(), recipe.getOutput(), false);
    ...
    private static class Remove extends BaseListRemoval<RecipeFurnace> {
        ...
        public void undo() {
            for (RecipeFurnace recipe : successful) {
                FurnaceManager.addRecipe(recipe.getEnergy(), recipe.getInput(), recipe.getOutput(), false);
            }
        }
