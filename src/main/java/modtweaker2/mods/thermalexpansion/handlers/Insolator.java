...
public class Insolator {
    ...
    @ZenMethod
    public static void addRecipe(int energy, IItemStack secondaryInput, IItemStack primaryInput,
            IItemStack primaryOutput, @Optional IItemStack secondaryOutput, @Optional int secondaryChance) {
        ...
        /*
         * TODO: Make it check for meta data if(InsolatorManager.recipeExists(toStack(primaryInput),
         * toStack(secondaryInput))) {
         * LogHelper.logWarning(String.format("Duplicate %s Recipe found for %s and %s. Command ignored!", name,
         * primaryInput.toString(), secondaryInput.toString())); return; }
         */

        RecipeInsolator recipe = ReflectionHelper.getInstance(
                ThermalHelper.insolatorRecipe,
                toStack(primaryInput),
                toStack(secondaryInput),
                toStack(primaryOutput),
                toStack(secondaryOutput),
                secondaryChance,
                energy);
    ...
    public static void removeRecipe(IIngredient input2, IIngredient input1) {
        ...
        for (RecipeInsolator recipe : InsolatorManager.getRecipeList()) {
            if (recipe != null && matches(input1, toIItemStack(recipe.getPrimaryInput()))
                    && matches(input2, toIItemStack(recipe.getSecondaryInput()))) {
                recipes.add(recipe);
            }
        }

        /*
         * TODO: Make it check for meta data if(!recipes.isEmpty()) { MineTweakerAPI.apply(new Remove(recipes)); } else
         * { LogHelper.logWarning(String.format("No %s Recipes found for %s and %s.", name, input1.toString(),
         * input2.toString())); }
         */
    }
