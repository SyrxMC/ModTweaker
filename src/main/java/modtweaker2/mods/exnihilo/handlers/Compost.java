...
public class Compost {
    ...
    public static void addRecipe(IItemStack input, double value, @Optional String hex) {
        ...
        recipes.put(new ItemInfo(toStack(input)), new Compostable(Math.min(1.0F, (float) value), new Color(hex)));
    ...
    public static void removeRecipe(IIngredient input) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s recipes found for %s. Command ignored!", Compost.name, input.toString()));
        }
