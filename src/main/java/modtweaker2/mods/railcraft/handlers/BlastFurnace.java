...
public class BlastFurnace {
    ...
    public static void addRecipe(IItemStack output, IItemStack input, int cookTime, @Optional Boolean matchEmptyNBT) {
        if (matchEmptyNBT == null) if (toStack(input).stackTagCompound == null) matchEmptyNBT = false;
        else matchEmptyNBT = true;

        if (input.getDamage() == OreDictionary.WILDCARD_VALUE) MineTweakerAPI.apply(
                new Add(
                        RailcraftHelper.getBlastFurnaceRecipe(
                                toStack(input),
                                false,
                                matchEmptyNBT,
                                cookTime,
    ...
    @Deprecated
    @ZenMethod
    public static void addRecipe(IItemStack input, boolean matchDamage, boolean matchNBT, int cookTime,
            IItemStack output) {
    ...
    public static void removeRecipe(IIngredient output) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", BlastFurnace.name, output.toString()));
        }
    }
