...
public class CokeOven {
     ...
     */
    @ZenMethod
    public static void addRecipe(IItemStack itemOutput, ILiquidStack fluidOutput, IItemStack ingredient,
            int timePerItem) {
        boolean matchEmptyNBT;
        if (toStack(ingredient).stackTagCompound == null) matchEmptyNBT = false;
        else matchEmptyNBT = true;

        if (ingredient.getDamage() == OreDictionary.WILDCARD_VALUE) MineTweakerAPI.apply(
                new Add(
                        new CokeOvenRecipe(
                                toStack(ingredient),
                                false,
    ...
    @ZenMethod
    public static void addRecipe(IItemStack itemOutput, ILiquidStack fluidOutput, IItemStack ingredient,
            boolean matchEmptyNBT, int timePerItem) {
        if (ingredient.getDamage() == OreDictionary.WILDCARD_VALUE) MineTweakerAPI.apply(
                new Add(
                        new CokeOvenRecipe(
                                toStack(ingredient),
    ...
    @Deprecated
    @ZenMethod
    public static void addRecipe(IItemStack input, boolean matchDamage, boolean matchNBT, IItemStack output,
            ILiquidStack fluidOutput, int cookTime) {
    ...
    public static void removeRecipe(IIngredient output) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", CokeOven.name, output.toString()));
        }
    }
