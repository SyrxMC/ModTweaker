...
public class Moistener {
    ...
    public static void removeRecipe(IIngredient output) {
        ...
        if (!recipes.isEmpty()) {
            ...
            LogHelper.logWarning(
                    String.format("No %s Recipe found for %s. Command ignored!", Moistener.name, output.toString()));
        }
    }
     ...
     * @param stage          What stage this product represents. Resources with lower stage value will be consumed
     *                       first. (First Stage is 0)
     */
    @ZenMethod
    public static void addFuel(IItemStack item, IItemStack product, int moistenerValue, int stage) {
        if (stage >= 0) {
            MineTweakerAPI
                    .apply(new AddFuel(new MoistenerFuel(toStack(item), toStack(product), moistenerValue, stage)));
        } else {
            LogHelper.logWarning(
                    String.format(
                            "No %s Recipe add for %s. Stage parameter must positive!",
                            Moistener.name,
                            item.toString()));
        }
    }
