...
public abstract class BaseMapAddition<K, V> extends BaseMapModification<K, V> {
        ...
        for (Entry<K, V> entry : recipes.entrySet()) {
            ...
            if (oldValue != null) {
                LogHelper.logWarning(
                        String.format(
                                "Overwritten %s Recipe for %s",
                                name,
                                getRecipeInfo(new AbstractMap.SimpleEntry<K, V>(entry.getKey(), value))));
    ...
    @Override
    public void undo() {
        if (successful.isEmpty() && overwritten.isEmpty()) return;
        ...
        for (Entry<K, V> entry : overwritten.entrySet()) {
            ...
            if (oldValue != null) {
                LogHelper.logWarning(
                        String.format(
                                "Overwritten %s Recipe which should not exist for %s",
                                name,
                                getRecipeInfo(new AbstractMap.SimpleEntry<K, V>(entry.getKey(), value))));
            }
        }
